...
export const SummaryCard: React.FC<SummaryCardProps> = ({ result, compareResult, lang, frame, lens, isLoading, isAdvanced = true }) => {
  ...
  const StatItem = ({ 
    ...
    return (
      <Card className="group bg-white dark:bg-slate-950 p-1.5 flex flex-col justify-between hover:shadow-sm dark:hover:shadow-md hover:shadow-slate-200/40 dark:hover:shadow-slate-900/80 hover:-translate-y-0.5 transition-all duration-300 h-full shadow-sm border border-slate-100 dark:border-slate-700/50 rounded-lg backdrop-blur-sm bg-linear-to-b from-white/98 dark:from-slate-950/98 to-white/95 dark:to-slate-950/95">
          ...
          <div className="flex items-baseline gap-1 flex-wrap group-hover:scale-y-[1.02] transition-transform duration-200 origin-left">
            {isLoading ? (
              ...
              <>
                ...
                {deltaNode && <div className="scale-75 origin-left shrink-0">{deltaNode}</div>}
              </>
            )}
          </div>
        </div>
    ...
    <div className="space-y-1.5 relative">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 min-w-0">
          <div className="w-0.5 h-4 bg-linear-to-b from-blue-400 to-blue-500 rounded-full shrink-0" />
        ...
        <div className="flex items-center gap-1 bg-slate-50/80 dark:bg-slate-800/60 p-1 rounded-lg border border-slate-200/50 dark:border-slate-700/50 shadow-sm backdrop-blur-sm shrink-0">
          {hasWarnings && (
            <button
              onClick={() => setActivePanel(p => p === 'warnings' ? 'none' : 'warnings')}
              className={`flex items-center gap-1 px-2 py-1 text-[9px] font-bold rounded-md transition-all duration-200 hover:scale-105 active:scale-95 ${
      ...
      <div
        className={`overflow-hidden transition-all duration-200 ease-out ${
          activePanel === 'warnings' && hasWarnings ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'
        }`}
      >
        {activePanel === 'warnings' && hasWarnings && (
          <div className="space-y-1.5 py-1.5">
            {isUnsafe && (
              ...
              >
                <div className="absolute -left-1 top-0 bottom-0 w-0.5 bg-linear-to-b from-red-500 to-rose-500 rounded-full" />
            ...
            {isHighDecentration && (
              ...
              >
                <div className="absolute -left-1 top-0 bottom-0 w-0.5 bg-linear-to-b from-amber-500 to-amber-400 rounded-full" />
      ...
      <div
        className={`overflow-hidden transition-all duration-200 ease-out ${
          activePanel === 'guide' ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'
        }`}
      >
        {activePanel === 'guide' && (
          <Card className={`grid gap-2 p-3 my-1 rounded-lg sm:grid-cols-2 ${isAdvanced ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} shadow-sm border-slate-200/60 dark:border-slate-700/50 bg-linear-to-br from-slate-50/60 dark:from-slate-900/40 to-slate-50/40 dark:to-slate-950/20 backdrop-blur-sm`}>
            <div
              className="space-y-1 bg-white dark:bg-slate-900/40 p-2 rounded-lg border border-slate-100/50 dark:border-slate-700/30 hover:shadow-sm transition-all duration-200 animate-fade-in-up"
              style={{ animationDelay: '0.05s', animationFillMode: 'both' }}
            >
