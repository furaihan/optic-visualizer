...
export const TopDownView: React.FC<TopDownViewProps> = ({
  ...
  const getLensPathTop = (
    ...
    const getPt = (lx: number, ly: number) =>
      `${centerX + lx * side * scale} ${centerY + ly * scale}`;
  ...
  return (
    <g>
      ...
      {[-1, 1].map((side) => (
        <g key={side}>
          <rect
            ...
            stroke={
              highlightedLimit === "a" ? "#f59e0b" : "url(#metallic-gradient)"
            }
          ...
          <g>
            ...
            <rect
              x={
                centerX +
                (side === 1 ? dbl / 2 + a - 1 : -dbl / 2 - a - 1) * scale
              }
              y={centerY + 0.5 * scale}
              width={2 * scale}
              height={3 * scale}
      ...
      <text
        x={centerX - (dbl / 2 + a / 2) * scale}
        y={centerY + depth * scale + 20}
        textAnchor="middle"
