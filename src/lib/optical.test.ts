...
import { calculateLens, LensParameters, FrameParameters, PatientParameters } from './optical';
...
describe('Optical Calculations - calculateLens', () => {
  ...
  it('calculates correct thicknesses under standard conditions', () => {
    const res = calculateLens(standardLens, standardFrame, standardPatient, 1.0, 0.33, 'full');
  ...
  it('safely handles extreme powers and returns finite numbers without NaN', () => {
    ...
    const res = calculateLens(extremeLens, standardFrame, standardPatient, 1.0, 0.33, 'full');
  ...
  it('generates index recommendation details matching guidelines', () => {
    ...
    const mildRes = calculateLens(mildLens, standardFrame, standardPatient, 1.0, 0.33, 'full');
    ...
    const heavyRes = calculateLens(heavyLens, standardFrame, standardPatient, 1.0, 0.33, 'full');
  ...
  it('calculates higher weight for lower index materials due to volume differences', () => {
    const baseRes15 = calculateLens({ ...standardLens, index: 1.50 }, standardFrame, standardPatient, 1.0, 0.33, 'full');
    const baseRes174 = calculateLens({ ...standardLens, index: 1.74 }, standardFrame, standardPatient, 1.0, 0.33, 'full');
...
describe('Clinical Rules & Input Validation - validateOpticalParams', () => {
  const validLens: LensParameters = { sph: -3.0, cyl: 0, axis: 0, index: 1.6, baseCurve: 5 };
  const validFrame: FrameParameters = { a: 50, b: 38, dbl: 18, depth: 4.0, ed: 52 };
  const validPatient: PatientParameters = { pd: 64, fittingHeight: 19 };

  it('passes fully compliant parameters', () => {
    const validation = validateOpticalParams(validLens, validFrame, validPatient);
  ...
  it('correctly clamps and snaps step inputs', () => {
    ...
    expect(clampAndSnapInput(-3.12, spec)).toBe(-3.00); 
    expect(clampAndSnapInput(-3.18, spec)).toBe(-3.25);
    expect(clampAndSnapInput(15.0, spec)).toBe(10.00); // clamped
  });
});
